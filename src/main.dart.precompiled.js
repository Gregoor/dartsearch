...
JSArray: {"": "List/Interceptor;",
  ...
  addAll$1: function(receiver, collection) {
    var t1;
    for (t1 = new H.ListIterator(collection, 1, 0, null); t1.moveNext$0();)
...
ListQueue: {"": "IterableBase;_table,_head,_tail,_modificationCount",
  ...
  addAll$1: function(_, elements) {
    ...
    t1 = J.getInterceptor$ax(elements);
  ...
  ListQueue$1: function(initialCapacity, $E) {
    ...
    if (initialCapacity != null) {
      if (typeof initialCapacity !== "number")
        throw initialCapacity.$lt();
      t1 = initialCapacity < 8;
    } else
      t1 = true;
    if (t1)
      initialCapacity = 8;
...
_StreamPool: {"": "Object;_controller,_subscriptions",
  add$1: function(_, stream) {
    ...
    t3 = t2.get$add(t2);
    t2.get$addError();
    t3 = new W._EventStreamSubscription(0, stream._target, stream._eventType, W._wrapZone(t3), stream._useCapture);
    H.setRuntimeTypeInfo(t3, [H.getRuntimeTypeArgument(stream, "_EventStream", 0)]);
    t3._tryResume$0();
    t1.$indexSet(t1, stream, t3);
  },
...
CanvasRenderingContext2D: {"": "CanvasRenderingContext;fillStyle},font},strokeStyle}",
...
Element: {"": "Node;className%",
  ...
  set$attributes: function(receiver, value) {
    var attributes, t1, t2, t3, key;
    attributes = new W._ElementAttributeMap(receiver);
    attributes.clear$0(attributes);
    for (t1 = value.get$keys(), t1 = new H.ListIterator(t1, t1.length, 0, null), t2 = value._element, t3 = attributes._element; t1.moveNext$0();) {
      key = t1._current;
      t3.setAttribute(key, t2.getAttribute(key));
    }
  },
...
InputElement: {"": "HtmlElement;files=,height%,name=,type%,value=,width%", $isElement: true, $isEventTarget: true, "%": "HTMLInputElement"},
...
["level", "models/level.dart", , G, {
Level: {"": "Object;fields,start,goal",
  initFields$2: function(width, height) {
    var t1;
    this.fields = P.List_List(width, null);
    t1 = this.fields;
    t1.toString;
    t1 = new H.MappedListIterable(t1, new G.Level_initFields_closure(height));
    H.setRuntimeTypeInfo(t1, [null, null]);
    this.fields = t1.toList$1$growable(t1, true);
  },
  adjacentFields$1: function(center) {
    var pos, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, $arguments;
    pos = center.get$pos();
    t1 = pos.x;
    ...
    t6 = t5.$add(t4, 1);
    t3 = H.Primitives_parseDouble(J.toStringAsFixed$1$n(t3, 3), null);
    t6 = H.Primitives_parseDouble(J.toStringAsFixed$1$n(t6, 3), null);
    t7 = t2.$add(t1, 0);
    t8 = t5.$add(t4, -1);
    t7 = H.Primitives_parseDouble(J.toStringAsFixed$1$n(t7, 3), null);
    t8 = H.Primitives_parseDouble(J.toStringAsFixed$1$n(t8, 3), null);
    t9 = t2.$add(t1, 1);
    t10 = t5.$add(t4, 0);
    t9 = H.Primitives_parseDouble(J.toStringAsFixed$1$n(t9, 3), null);
    t10 = H.Primitives_parseDouble(J.toStringAsFixed$1$n(t10, 3), null);
    t1 = t2.$add(t1, -1);
    t4 = t5.$add(t4, 0);
    t4 = new H.WhereIterable([new U.Vector(t3, t6, null), new U.Vector(t7, t8, null), new U.Vector(t9, t10, null), new U.Vector(H.Primitives_parseDouble(J.toStringAsFixed$1$n(t1, 3), null), H.Primitives_parseDouble(J.toStringAsFixed$1$n(t4, 3), null), null)], new G.Level_adjacentFields_closure(this));
    t4.$builtinTypeInfo = [null];
    $arguments = H.substitute(t4.$asIterableBase, H.getRuntimeTypeInfo(t4));
    t1 = $arguments == null ? null : $arguments[0];
    return H.MappedIterable_MappedIterable(t4, new G.Level_adjacentFields_closure0(this), t1, null);
  },
  ...
  Level$fromAsciiString$1: function(string) {
    var rows, width, height, x, y, t1, field, type;
    rows = J.split$1$s(string, "\n");
    if (0 >= rows.length)
      throw H.ioore(rows, 0);
    width = J.get$length$asx(rows[0]);
    height = rows.length;
    this.initFields$2(width, height);
    x = 0;
    while (true) {
      ...
      for (y = 0; y < height; ++y) {
        ...
        if (x >= t1.length)
          throw H.ioore(t1, x);
        field = new G.Field(new U.Vector(H.Primitives_parseDouble(C.JSInt_methods.toStringAsFixed$1(x, 3), null), H.Primitives_parseDouble(C.JSInt_methods.toStringAsFixed$1(y, 3), null), null), null, false);
        J.$indexSet$ax(t1[x], y, field);
        if (y >= rows.length)
          throw H.ioore(rows, y);
        switch (J.$index$asx(rows[y], x)) {
          case " ":
            type = C.FieldType_0;
            break;
          case "x":
          ...
          case "g":
            ...
            break;
          default:
            throw H.wrapException(P.FormatException$("Invalid field type"));
        }
        field.type = type;
      }
      ++x;
  ...
  Level$random$2: function(width, height) {
    var x, x0, t1, y, t2, type, field, $arguments, adjacentWalls, missingFields, max, max0, t3;
    this.initFields$2(width, height);
    x = 0;
    while (true) {
      if (typeof width !== "number")
      ...
      for (; y < height; ++y) {
        if (P.min(x, y) !== 0)
          ...
          t2 = true;
        type = t2 ? C.FieldType_1 : C.FieldType_0;
        t2 = this.fields;
        if (x >= t2.length)
          throw H.ioore(t2, x);
        J.$indexSet$ax(t2[x], y, new G.Field(new U.Vector(H.Primitives_parseDouble(C.JSInt_methods.toStringAsFixed$1(x, 3), null), H.Primitives_parseDouble(C.JSInt_methods.toStringAsFixed$1(y, 3), null), null), type, false));
      }
      x = x0;
    }
    for (x = 0; x < width; ++x) {
      ...
      for (; y < height; ++y) {
        ...
        if (x >= t1.length)
          throw H.ioore(t1, x);
        field = J.$index$asx(t1[x], y);
        t1 = this.adjacentFields$1(field);
        $arguments = H.substitute(t1.$asIterableBase, H.getRuntimeTypeInfo(t1));
        t2 = $arguments == null ? null : $arguments[0];
        t1 = new H.WhereIterable(t1, new G.Level$random_closure());
        t1.$builtinTypeInfo = [t2];
        adjacentWalls = t1.get$length(t1);
        if (adjacentWalls !== 4)
          t1 = adjacentWalls === 2 && adjacentWalls * Math.random() > 0.7;
        else
          t1 = true;
        if (t1)
          J.set$type$x(field, C.FieldType_1);
      }
    }
    missingFields = [C.FieldType_2, C.FieldType_3];
    for (max = width - 2, t1 = J.getInterceptor$n(height); missingFields.length !== 0;) {
      type = C.JSArray_methods.get$first(missingFields);
      do {
        t2 = this.fields;
        if (max < 0)
          H.throwExpression(new P.ArgumentError("negative max: " + H.S(max)));
        max0 = max > 4294967295 ? 4294967295 : max;
        t3 = (Math.random() * max0 >>> 0) + 1;
        if (t3 < 0 || t3 >= t2.length)
          throw H.ioore(t2, t3);
        t3 = t2[t3];
        max0 = t1.$sub(height, 2);
        t2 = J.getInterceptor$n(max0);
        if (t2.$lt(max0, 0))
          H.throwExpression(new P.ArgumentError("negative max: " + H.S(max0)));
        if (t2.$gt(max0, 4294967295))
          max0 = 4294967295;
        field = J.$index$asx(t3, (Math.random() * max0 >>> 0) + 1);
        t2 = J.getInterceptor$x(field);
        if (t2.get$type(field) === C.FieldType_0) {
          t3 = this.adjacentFields$1(field);
          t3 = t3.any$1(t3, new G.Level$random_closure0());
        } else
          t3 = true;
      } while (t3);
      t2.set$type(field, type);
      switch (type) {
        case C.FieldType_2:
          this.start = field;
          break;
      ...
      C.JSArray_methods.remove$1(missingFields, type);
    }
  },
  static: {
Level$fromAsciiString: function(string) {
  var t1 = new G.Level(null, null, null);
  t1.Level$fromAsciiString$1(string);
  return t1;
},

Level$random: function(width, height) {
  var t1 = new G.Level(null, null, null);
  t1.Level$random$2(width, height);
  return t1;
}}

},

Level$random_closure: {"": "Closure;",
  call$1: function(field) {
    return J.get$type$x(field) === C.FieldType_1;
  },
  $is_args1: true
},

Level$random_closure0: {"": "Closure;",
  call$1: function(neighbour) {
    return C.JSArray_methods.contains$1([C.FieldType_3, C.FieldType_2], J.get$type$x(neighbour));
  },
  $is_args1: true
},

Level_initFields_closure: {"": "Closure;height_0",
  call$1: function(el) {
    return P.List_List(this.height_0, null);
  },
  $is_args1: true
},

Level_adjacentFields_closure: {"": "Closure;this_0",
  call$1: function(v) {
    var t1, x, y, t2;
    ...
    if (J.$gt$n(t1.$mul(x, y), 0)) {
      t2 = this.this_0;
      if (t1.$lt(x, t2.fields.length)) {
        t1 = t2.fields;
        if (0 >= t1.length)
          throw H.ioore(t1, 0);
        t1 = J.$lt$n(y, J.get$length$asx(t1[0]));
      } else
        t1 = false;
    } else
      t1 = false;
...
Level_adjacentFields_closure0: {"": "Closure;this_1",
  call$1: function(pos) {
    var t1, t2, t3;
    t1 = this.this_1.fields;
    t2 = J.getInterceptor$x(pos);
    t3 = J.toInt$0$n(t2.get$x(pos));
    if (t3 < 0 || t3 >= t1.length)
      throw H.ioore(t1, t3);
    return J.$index$asx(t1[t3], J.toInt$0$n(t2.get$y(pos)));
  },
  $is_args1: true
},
...
FieldType: {"": "Object;value",
  ...
  static: {
"": "FieldType_EMPTY,FieldType_WALL,FieldType_START,FieldType_GOAL",
}

},

Field: {"": "Object;pos<,type*,visited",
  toString$0: function(_) {
    return H.S(this.pos);
  }
}}],
["", "main.dart", , F, {
main: function() {
  var t1, t2, container, canvasContainer;
...
resize: function() {
  H.IterableMixinWorkaround_forEach($.get$renderers(), new F.resize_closure());
  F.render();
},

render: function() {
  H.IterableMixinWorkaround_forEach($.get$renderers(), new F.render_closure());
},
...
resize_closure: {"": "Closure;",
  call$1: function(renderer) {
    ...
    if (typeof t2 !== "number")
      throw t2.$sub();
    J.resize$2$x(renderer, t1, t2 - (t3 + 10));
    renderer.set$redraw(true);
  },
  $is_args1: true
},
...
1],
["", "views/menu.dart", , D, {
attachMenuListeners: function(container, onClearSearch, onLoadLevel, onRandomLevel, onSearch) {
  var searchBar, showSearchBar, widthInput, heightInput, t1, t2, t3, reader, levelUpload;
  searchBar = container.querySelector(".searchBar");
  showSearchBar = new D.attachMenuListeners_closure(searchBar);
  widthInput = container.querySelector("input.width");
  heightInput = container.querySelector("input.height");
  t1 = container.querySelector("button.randomLevel");
  t1.toString;
  t2 = C.EventStreamProvider_click._eventType;
  t1 = new W._ElementEventStreamImpl(t1, t2, false);
  H.setRuntimeTypeInfo(t1, [null]);
  t3 = new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new D.attachMenuListeners_closure0(onRandomLevel, showSearchBar, widthInput, heightInput)), t1._useCapture);
  H.setRuntimeTypeInfo(t3, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
  t3._tryResume$0();
  reader = new FileReader();
  levelUpload = container.querySelector("input.uploadLevel");
  levelUpload.toString;
  t3 = new W._ElementEventStreamImpl(levelUpload, C.EventStreamProvider_change._eventType, false);
  H.setRuntimeTypeInfo(t3, [null]);
  t1 = new W._EventStreamSubscription(0, t3._target, t3._eventType, W._wrapZone(new D.attachMenuListeners_closure1(reader, levelUpload)), t3._useCapture);
  ...
  H.setRuntimeTypeInfo(t1, [null]);
  t3 = new W._EventStreamSubscription(0, t1._target, t1._eventType, W._wrapZone(new D.attachMenuListeners_closure2(onLoadLevel, showSearchBar, reader)), t1._useCapture);
  H.setRuntimeTypeInfo(t3, [H.getRuntimeTypeArgument(t1, "_EventStream", 0)]);
  t3._tryResume$0();
  t2 = new W._ElementListEventStreamImpl(W._FrozenElementList$_wrap(searchBar.querySelectorAll("button"), null), false, t2);
  H.setRuntimeTypeInfo(t2, [null]);
  t2.listen$1(new D.attachMenuListeners_closure3(onSearch, onClearSearch));
  $.menuSize = container.scrollHeight;
},

attachMenuListeners_closure: {"": "Closure;searchBar_0",
  call$0: function() {
    var t1 = J.get$classes$x(this.searchBar_0);
    return t1.remove$1(t1, "hidden");
  }
},

attachMenuListeners_closure0: {"": "Closure;onRandomLevel_1,showSearchBar_2,widthInput_3,heightInput_4",
  call$1: function(e) {
    this.showSearchBar_2.call$0();
    this.onRandomLevel_1.call$2(H.Primitives_parseInt(J.get$value$x(this.widthInput_3), null, null), H.Primitives_parseInt(J.get$value$x(this.heightInput_4), null, null));
  },
  $is_args1: true
},

attachMenuListeners_closure1: {"": "Closure;reader_5,levelUpload_6",
  call$1: function(e) {
    var t1 = J.get$files$x(this.levelUpload_6);
    if (0 >= t1.length)
      throw H.ioore(t1, 0);
    return this.reader_5.readAsText(t1[0].slice());
  },
  $is_args1: true
},

attachMenuListeners_closure2: {"": "Closure;onLoadLevel_7,showSearchBar_8,reader_9",
  call$1: function(e) {
    this.showSearchBar_8.call$0();
    this.onLoadLevel_7.call$1(this.reader_9.result);
  },
  $is_args1: true
},

attachMenuListeners_closure3: {"": "Closure;onSearch_10,onClearSearch_11",
  call$1: function($event) {
    var el, t1, t2;
    el = J.get$currentTarget$x($event);
    t1 = J.getInterceptor$x(el);
    t2 = t1.get$classes(el).readClasses$0();
    if (t2.contains$1(t2, "clear"))
      this.onClearSearch_11.call$0();
    else
      this.onSearch_10.call$1(t1.get$attributes(el)._element.getAttribute("data-search"));
  },
  $is_args1: true
}}],
["renderers", "views/renderer.dart", , Y, {
Renderer: {"": "Object;redraw@",
  clearCanvas$1: function(canvas) {
    var t1 = J.getInterceptor$x(canvas);
    J.clearRect$4$x(t1.getContext$1(canvas, "2d"), 0, 0, t1.get$width(canvas), t1.get$height(canvas));
  }
},

LevelRenderer: {"": "Renderer;level,parent,canvas,ctx,scalingFactor,redraw",
  render$0: function() {
    var t1, x, y, t2, f, t3, t4, scaledPos, fillColor;
    this.clearCanvas$1(this.canvas);
    for (t1 = this.level, x = 0; x < t1.fields.length; ++x) {
      y = 0;
      while (true) {
        t2 = t1.fields;
        if (0 >= t2.length)
        ...
        if (x >= t2.length)
          throw H.ioore(t2, x);
        f = J.$index$asx(t2[x], y);
        t2 = f.get$pos();
        t3 = this.scalingFactor;
        t4 = J.$mul$n(t2.x, t3);
        t3 = J.$mul$n(t2.y, t3);
        scaledPos = new U.Vector(H.Primitives_parseDouble(J.toStringAsFixed$1$n(t4, 3), null), H.Primitives_parseDouble(J.toStringAsFixed$1$n(t3, 3), null), null);
        switch (J.get$type$x(f)) {
          case C.FieldType_0:
            J.set$strokeStyle$x(this.ctx, "black");
            t2 = this.ctx;
            t3 = this.scalingFactor;
            J.strokeRect$4$x(t2, scaledPos.x, scaledPos.y, t3, t3);
            fillColor = "white";
            break;
          case C.FieldType_2:
            fillColor = "lightgrey";
        ...
        t3.set$fillStyle(t2, fillColor);
        t4 = this.scalingFactor;
        t3.fillRect$4(t2, scaledPos.x, scaledPos.y, t4, t4);
        ++y;
      }
    }
    this.redraw = false;
  ...
  resize$2: function(_, width, height) {
    ...
    if (typeof t2 !== "number")
      throw H.iae(t2);
    this.scalingFactor = P.min(width / t3, height / t2);
    t2 = this.parent.style;
    t3 = this.scalingFactor;
    t4 = t1.fields.length;
    if (typeof t3 !== "number")
      throw t3.$mul();
    t5 = J.getInterceptor$x(t2);
    t5.set$width(t2, H.S(t3 * t4) + "px");
    t4 = this.scalingFactor;
    t3 = t1.fields;
    if (0 >= t3.length)
      throw H.ioore(t3, 0);
    ...
    if (typeof t2 !== "number")
      throw H.iae(t2);
    t3.setAttribute("height", H.S(t4 * t2));
    t2 = this.scalingFactor;
    t1 = t1.fields.length;
    if (typeof t2 !== "number")
      throw t2.$mul();
    t3.setAttribute("width", H.S(t2 * t1));
    J.set$position$x(this.canvas.style, "absolute");
  },
  remove$0: function(_) {
    J.remove$0$ax(this.canvas);
    return true;
  },
  LevelRenderer$2: function($parent, level) {
    this.canvas = W.CanvasElement_CanvasElement(0, 0);
    this.ctx = J.getContext$1$x(this.canvas, "2d");
    this.parent.appendChild(this.canvas);
  },
  static: {
LevelRenderer$: function($parent, level) {
  var t1 = new Y.LevelRenderer(level, $parent, null, null, null, true);
  t1.LevelRenderer$2($parent, level);
  return t1;
}}

},

PathRenderer: {"": "Renderer;path,levelRenderer,fieldCanvas,numberCanvas,canvases,redraw",
  render$0: function() {
    var t1 = {};
    H.IterableMixinWorkaround_forEach(this.canvases, new Y.PathRenderer_render_closure(this));
    t1.ctx_0 = null;
    t1.i_1 = 0;
    H.IterableMixinWorkaround_forEach(this.path, new Y.PathRenderer_render_closure0(t1, this));
  },
  resize$2: function(_, w, h) {
    H.IterableMixinWorkaround_forEach(this.canvases, new Y.PathRenderer_resize_closure(this));
  },
  remove$0: function(_) {
    H.IterableMixinWorkaround_forEach(this.canvases, new Y.PathRenderer_remove_closure());
    return true;
  },
  PathRenderer$3: function($parent, levelRenderer, path) {
    var t1;
    this.numberCanvas = this.levelRenderer.canvas.cloneNode(true);
    this.fieldCanvas = J.clone$1$x(this.numberCanvas, true);
    t1 = [this.fieldCanvas, this.numberCanvas];
...
PathRenderer$: function($parent, levelRenderer, path) {
  ...
  t1.PathRenderer$3($parent, levelRenderer, path);
  return t1;
}}

},

PathRenderer_closure: {"": "Closure;parent_0",
  call$1: function(canvas) {
    return this.parent_0.appendChild(canvas);
  },
  $is_args1: true
},

PathRenderer_render_closure: {"": "Closure;this_1",
  call$1: function(canvas) {
    return this.this_1.clearCanvas$1(canvas);
  },
  $is_args1: true
},

PathRenderer_render_closure0: {"": "Closure;box_0,this_2",
  call$1: function(f) {
    var t1, t2, t3, t4, fieldPos, start, end, percentFade, t5, diffRed, diffGreen, diffBlue, t6, text, textPos;
    t1 = f.get$pos();
    t2 = this.this_2;
    t3 = t2.levelRenderer;
    t4 = t3.scalingFactor;
    fieldPos = U.Vector$(J.$mul$n(t1.x, t4), J.$mul$n(t1.y, t4));
    t1 = this.box_0;
    if (t1.i_1 > 0) {
      start = H.fillLiteralMap(["red", 0, "green", 0, "blue", 255], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
      end = H.fillLiteralMap(["red", 255, "green", 0, "blue", 0], P.LinkedHashMap_LinkedHashMap(null, null, null, null, null));
      percentFade = t1.i_1 / t2.path.length;
      t4 = start.$index(start, "red");
      t5 = J.$sub$n(end.$index(end, "red"), start.$index(start, "red"));
      if (typeof t5 !== "number")
        throw H.iae(t5);
      diffRed = J.$add$ns(t4, percentFade * t5);
      t5 = start.$index(start, "green");
      t4 = J.$sub$n(end.$index(end, "green"), start.$index(start, "green"));
      if (typeof t4 !== "number")
        throw H.iae(t4);
      diffGreen = J.$add$ns(t5, percentFade * t4);
      t4 = start.$index(start, "blue");
      t5 = J.$sub$n(end.$index(end, "blue"), start.$index(start, "blue"));
      if (typeof t5 !== "number")
        throw H.iae(t5);
      diffBlue = J.$add$ns(t4, percentFade * t5);
      t5 = J.getContext$1$x(t2.fieldCanvas, "2d");
      t4 = J.getInterceptor$x(t5);
      t4.set$fillStyle(t5, "rgb(" + H.S(J.round$0$n(diffRed)) + ", " + H.S(J.round$0$n(diffGreen)) + ", " + H.S(J.round$0$n(diffBlue)) + ")");
      t6 = t3.scalingFactor;
      t4.fillRect$4(t5, fieldPos.x, fieldPos.y, t6, t6);
    }
    t1.ctx_0 = J.getContext$1$x(t2.numberCanvas, "2d");
    text = "" + t1.i_1;
    if (t1.i_1 === 0)
      text = "S";
    t2 = t3.scalingFactor;
    if (typeof t2 !== "number")
      throw t2.$div();
    t4 = J.measureText$1$x(t1.ctx_0, text).width;
    if (typeof t4 !== "number")
      throw t4.$div();
    t5 = t3.scalingFactor;
    if (typeof t5 !== "number")
      throw t5.$div();
    t6 = J.measureText$1$x(t1.ctx_0, "M").width;
    if (typeof t6 !== "number")
      throw t6.$div();
    textPos = U.Vector$(J.$add$ns(fieldPos.x, t2 / 2 - t4 / 2), J.$add$ns(fieldPos.y, t5 / 2 + t6 / 2));
    t6 = t1.ctx_0;
    t3 = t3.scalingFactor;
    if (typeof t3 !== "number")
      throw t3.$div();
    t5 = J.getInterceptor$x(t6);
    t5.set$font(t6, H.S(t3 / 2) + "px Arial");
    t5.fillText$3(t6, text, textPos.x, textPos.y);
    t1.i_1 = t1.i_1 + 1;
  },
  $is_args1: true
},

PathRenderer_resize_closure: {"": "Closure;this_0",
  call$1: function(canvas) {
    var t1 = this.this_0.levelRenderer.canvas;
    t1.toString;
    t1 = new W._ElementAttributeMap(t1);
    J.set$attributes$x(canvas, t1);
    return t1;
  },
  $is_args1: true
},

PathRenderer_remove_closure: {"": "Closure;",
  call$1: function(canvas) {
    return J.remove$0$ax(canvas);
  },
  $is_args1: true
}}],
...
Search: {"": "Object;",
  findPath$0: function() {
    ...
    while (true) {
      ...
      neighbours = P.List_List$from(t3, true, t4);
      H.IterableMixinWorkaround_shuffleList(neighbours, null);
      t4 = this.frontier;
  ...
  Search$1: function(level) {
    this.frontier = P.ListQueue_ListQueue$from([this.level.start], null);
    this.findPath$0();
  }
},

Search_findPath_closure: {"": "Closure;this_0",
  call$1: function(f) {
    return J.get$type$x(f) !== C.FieldType_1 && !C.JSArray_methods.contains$1(this.this_0.path, f);
  },
  $is_args1: true
},
...
["vector", "models/vector.dart", , U, {
Vector: {"": "Object;x>,y>,_vector$_length",
  $eq: function(_, v) {
    var t1;
    if (v == null)
      return false;
    t1 = J.getInterceptor$x(v);
    return J.$eq(this.x, t1.get$x(v)) && J.$eq(this.y, t1.get$y(v));
  },
  $gt: function(_, v) {
    var t1, t2;
  ...
  $add: function(_, v) {
    ...
    return U.Vector$(J.$add$ns(this.x, t1.get$x(v)), J.$add$ns(this.y, t1.get$y(v)));
  },
  $sub: function(_, v) {
    var t1 = J.getInterceptor$x(v);
    return U.Vector$(J.$sub$n(this.x, t1.get$x(v)), J.$sub$n(this.y, t1.get$y(v)));
  },
  $mul: function(_, v) {
    var t1 = J.getInterceptor$x(v);
    return U.Vector$(J.$mul$n(this.x, t1.get$x(v)), J.$mul$n(this.y, t1.get$y(v)));
  },
  get$length: function(_) {
    var t1, t2;
    if (this._vector$_length == null) {
      t1 = this.x;
      if (typeof t1 !== "number")
      ...
      t2 = Math.pow(t2, 2);
      this._vector$_length = Math.sqrt(t1 + t2);
    }
    return this._vector$_length;
  },
  get$hashCode: function(_) {
    return J.$add$ns(this.x, this.y);
  },
  toString$0: function(_) {
    return H.S(this.x) + " / " + H.S(this.y);
  },
  round$0: function(_) {
    return U.Vector$(J.round$0$n(this.x), J.round$0$n(this.y));
  },
  static: {
Vector$: function(x, y) {
  return new U.Vector(H.Primitives_parseDouble(J.toStringAsFixed$1$n(x, 3), null), H.Primitives_parseDouble(J.toStringAsFixed$1$n(y, 3), null), null);
}}
...
G.Field.$isField = true;
G.Field.$isObject = true;
U.Vector.$isVector = true;
...
C.FieldType_0 = new G.FieldType(0);
C.FieldType_1 = new G.FieldType(1);
C.FieldType_2 = new G.FieldType(2);
C.FieldType_3 = new G.FieldType(3);
...
Isolate.$lazy($, "renderers", "renderers", "get$renderers", function() {
  return [];
});
// Native classes
J.Interceptor["%"] = "ArrayBuffer|CanvasGradient|CanvasPattern|DOMError|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedEnumeration|SVGAnimatedLength|SVGAnimatedLengthList|SVGAnimatedNumber|SVGAnimatedNumberList|SVGAnimatedString";
...
function dart_precompiled($collectedClasses) {
  ...
  function Level(fields, start, goal) {
    this.fields = fields;
    this.start = start;
    this.goal = goal;
  }
  ...
  function Level$random_closure() {
  }
  Level$random_closure.builtin$cls = "Level$random_closure";
  if (!"name" in Level$random_closure)
    Level$random_closure.name = "Level$random_closure";
  $desc = $collectedClasses.Level$random_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Level$random_closure.prototype = $desc;
  function Level$random_closure0() {
  }
  Level$random_closure0.builtin$cls = "Level$random_closure0";
  if (!"name" in Level$random_closure0)
    Level$random_closure0.name = "Level$random_closure0";
  $desc = $collectedClasses.Level$random_closure0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Level$random_closure0.prototype = $desc;
  function Level_initFields_closure(height_0) {
    this.height_0 = height_0;
  }
  Level_initFields_closure.builtin$cls = "Level_initFields_closure";
  if (!"name" in Level_initFields_closure)
    Level_initFields_closure.name = "Level_initFields_closure";
  $desc = $collectedClasses.Level_initFields_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Level_initFields_closure.prototype = $desc;
  function Level_adjacentFields_closure(this_0) {
    this.this_0 = this_0;
  }
  Level_adjacentFields_closure.builtin$cls = "Level_adjacentFields_closure";
  if (!"name" in Level_adjacentFields_closure)
    Level_adjacentFields_closure.name = "Level_adjacentFields_closure";
  $desc = $collectedClasses.Level_adjacentFields_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Level_adjacentFields_closure.prototype = $desc;
  function Level_adjacentFields_closure0(this_1) {
    this.this_1 = this_1;
  }
  Level_adjacentFields_closure0.builtin$cls = "Level_adjacentFields_closure0";
  if (!"name" in Level_adjacentFields_closure0)
    Level_adjacentFields_closure0.name = "Level_adjacentFields_closure0";
  $desc = $collectedClasses.Level_adjacentFields_closure0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Level_adjacentFields_closure0.prototype = $desc;
  function FieldType(value) {
    this.value = value;
  }
  FieldType.builtin$cls = "FieldType";
  if (!"name" in FieldType)
    FieldType.name = "FieldType";
  $desc = $collectedClasses.FieldType;
  if ($desc instanceof Array)
    $desc = $desc[1];
  FieldType.prototype = $desc;
  function Field(pos, type, visited) {
    this.pos = pos;
    this.type = type;
  ...
  if (!"name" in Field)
    Field.name = "Field";
  $desc = $collectedClasses.Field;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Field.prototype = $desc;
  Field.prototype.get$pos = function() {
    return this.pos;
  };
  Field.prototype.get$type = function(receiver) {
    return this.type;
  };
  Field.prototype.set$type = function(receiver, v) {
    return this.type = v;
  };
  ...
  function main_removePathRenderer() {
  }
  main_removePathRenderer.builtin$cls = "main_removePathRenderer";
  if (!"name" in main_removePathRenderer)
    main_removePathRenderer.name = "main_removePathRenderer";
  $desc = $collectedClasses.main_removePathRenderer;
  if ($desc instanceof Array)
    $desc = $desc[1];
  main_removePathRenderer.prototype = $desc;
  function main_removePathRenderer_closure() {
  }
  main_removePathRenderer_closure.builtin$cls = "main_removePathRenderer_closure";
  if (!"name" in main_removePathRenderer_closure)
    main_removePathRenderer_closure.name = "main_removePathRenderer_closure";
  $desc = $collectedClasses.main_removePathRenderer_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  main_removePathRenderer_closure.prototype = $desc;
  function main_initLevelRenderer(canvasContainer_0, removePathRenderer_1) {
    this.canvasContainer_0 = canvasContainer_0;
    this.removePathRenderer_1 = removePathRenderer_1;
  }
  main_initLevelRenderer.builtin$cls = "main_initLevelRenderer";
  if (!"name" in main_initLevelRenderer)
    main_initLevelRenderer.name = "main_initLevelRenderer";
  $desc = $collectedClasses.main_initLevelRenderer;
  if ($desc instanceof Array)
    $desc = $desc[1];
  main_initLevelRenderer.prototype = $desc;
  function main_closure1(initLevelRenderer_2) {
    this.initLevelRenderer_2 = initLevelRenderer_2;
  }
  ...
  function main_closure0(initLevelRenderer_3) {
    this.initLevelRenderer_3 = initLevelRenderer_3;
  }
  ...
  function main_closure2(canvasContainer_4, removePathRenderer_5) {
    this.canvasContainer_4 = canvasContainer_4;
    this.removePathRenderer_5 = removePathRenderer_5;
  }
  ...
  function resize_closure() {
  }
  resize_closure.builtin$cls = "resize_closure";
  if (!"name" in resize_closure)
    resize_closure.name = "resize_closure";
  $desc = $collectedClasses.resize_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  resize_closure.prototype = $desc;
  function render_closure() {
  }
  ...
  function attachMenuListeners_closure(searchBar_0) {
    this.searchBar_0 = searchBar_0;
  }
  attachMenuListeners_closure.builtin$cls = "attachMenuListeners_closure";
  if (!"name" in attachMenuListeners_closure)
    attachMenuListeners_closure.name = "attachMenuListeners_closure";
  $desc = $collectedClasses.attachMenuListeners_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  attachMenuListeners_closure.prototype = $desc;
  function attachMenuListeners_closure0(onRandomLevel_1, showSearchBar_2, widthInput_3, heightInput_4) {
    this.onRandomLevel_1 = onRandomLevel_1;
    this.showSearchBar_2 = showSearchBar_2;
    this.widthInput_3 = widthInput_3;
    this.heightInput_4 = heightInput_4;
  }
  attachMenuListeners_closure0.builtin$cls = "attachMenuListeners_closure0";
  if (!"name" in attachMenuListeners_closure0)
    attachMenuListeners_closure0.name = "attachMenuListeners_closure0";
  $desc = $collectedClasses.attachMenuListeners_closure0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  attachMenuListeners_closure0.prototype = $desc;
  function attachMenuListeners_closure1(reader_5, levelUpload_6) {
    this.reader_5 = reader_5;
    this.levelUpload_6 = levelUpload_6;
  }
  attachMenuListeners_closure1.builtin$cls = "attachMenuListeners_closure1";
  if (!"name" in attachMenuListeners_closure1)
    attachMenuListeners_closure1.name = "attachMenuListeners_closure1";
  $desc = $collectedClasses.attachMenuListeners_closure1;
  if ($desc instanceof Array)
    $desc = $desc[1];
  attachMenuListeners_closure1.prototype = $desc;
  function attachMenuListeners_closure2(onLoadLevel_7, showSearchBar_8, reader_9) {
    this.onLoadLevel_7 = onLoadLevel_7;
    this.showSearchBar_8 = showSearchBar_8;
    this.reader_9 = reader_9;
  }
  attachMenuListeners_closure2.builtin$cls = "attachMenuListeners_closure2";
  if (!"name" in attachMenuListeners_closure2)
    attachMenuListeners_closure2.name = "attachMenuListeners_closure2";
  $desc = $collectedClasses.attachMenuListeners_closure2;
  if ($desc instanceof Array)
    $desc = $desc[1];
  attachMenuListeners_closure2.prototype = $desc;
  function attachMenuListeners_closure3(onSearch_10, onClearSearch_11) {
    this.onSearch_10 = onSearch_10;
    this.onClearSearch_11 = onClearSearch_11;
  }
  attachMenuListeners_closure3.builtin$cls = "attachMenuListeners_closure3";
  if (!"name" in attachMenuListeners_closure3)
    attachMenuListeners_closure3.name = "attachMenuListeners_closure3";
  $desc = $collectedClasses.attachMenuListeners_closure3;
  if ($desc instanceof Array)
    $desc = $desc[1];
  attachMenuListeners_closure3.prototype = $desc;
  ...
  Renderer.prototype.get$redraw = function() {
    return this.redraw;
  };
  Renderer.prototype.set$redraw = function(v) {
    return this.redraw = v;
  };
  function LevelRenderer(level, parent, canvas, ctx, scalingFactor, redraw) {
    this.level = level;
    this.parent = parent;
    this.canvas = canvas;
    this.ctx = ctx;
  ...
  LevelRenderer.builtin$cls = "LevelRenderer";
  if (!"name" in LevelRenderer)
    LevelRenderer.name = "LevelRenderer";
  $desc = $collectedClasses.LevelRenderer;
  if ($desc instanceof Array)
    $desc = $desc[1];
  LevelRenderer.prototype = $desc;
  function PathRenderer(path, levelRenderer, fieldCanvas, numberCanvas, canvases, redraw) {
    this.path = path;
  ...
  PathRenderer.builtin$cls = "PathRenderer";
  if (!"name" in PathRenderer)
    PathRenderer.name = "PathRenderer";
  $desc = $collectedClasses.PathRenderer;
  if ($desc instanceof Array)
    $desc = $desc[1];
  PathRenderer.prototype = $desc;
  function PathRenderer_closure(parent_0) {
    this.parent_0 = parent_0;
  }
  PathRenderer_closure.builtin$cls = "PathRenderer_closure";
  if (!"name" in PathRenderer_closure)
    PathRenderer_closure.name = "PathRenderer_closure";
  $desc = $collectedClasses.PathRenderer_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  PathRenderer_closure.prototype = $desc;
  function PathRenderer_render_closure(this_1) {
    this.this_1 = this_1;
  }
  PathRenderer_render_closure.builtin$cls = "PathRenderer_render_closure";
  if (!"name" in PathRenderer_render_closure)
    PathRenderer_render_closure.name = "PathRenderer_render_closure";
  $desc = $collectedClasses.PathRenderer_render_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  PathRenderer_render_closure.prototype = $desc;
  function PathRenderer_render_closure0(box_0, this_2) {
    this.box_0 = box_0;
    this.this_2 = this_2;
  }
  PathRenderer_render_closure0.builtin$cls = "PathRenderer_render_closure0";
  if (!"name" in PathRenderer_render_closure0)
    PathRenderer_render_closure0.name = "PathRenderer_render_closure0";
  $desc = $collectedClasses.PathRenderer_render_closure0;
  if ($desc instanceof Array)
    $desc = $desc[1];
  PathRenderer_render_closure0.prototype = $desc;
  function PathRenderer_resize_closure(this_0) {
    this.this_0 = this_0;
  }
  PathRenderer_resize_closure.builtin$cls = "PathRenderer_resize_closure";
  if (!"name" in PathRenderer_resize_closure)
    PathRenderer_resize_closure.name = "PathRenderer_resize_closure";
  $desc = $collectedClasses.PathRenderer_resize_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  PathRenderer_resize_closure.prototype = $desc;
  function PathRenderer_remove_closure() {
  }
  PathRenderer_remove_closure.builtin$cls = "PathRenderer_remove_closure";
  if (!"name" in PathRenderer_remove_closure)
    PathRenderer_remove_closure.name = "PathRenderer_remove_closure";
  $desc = $collectedClasses.PathRenderer_remove_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  PathRenderer_remove_closure.prototype = $desc;
  function Search() {
  }
  ...
  function Search_findPath_closure(this_0) {
    this.this_0 = this_0;
  }
  Search_findPath_closure.builtin$cls = "Search_findPath_closure";
  if (!"name" in Search_findPath_closure)
    Search_findPath_closure.name = "Search_findPath_closure";
  $desc = $collectedClasses.Search_findPath_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Search_findPath_closure.prototype = $desc;
  function BreadthFirst(goalReached, unreachable, level, frontier, path) {
  ...
  BreadthFirst.builtin$cls = "BreadthFirst";
  if (!"name" in BreadthFirst)
    BreadthFirst.name = "BreadthFirst";
  $desc = $collectedClasses.BreadthFirst;
  if ($desc instanceof Array)
    $desc = $desc[1];
  BreadthFirst.prototype = $desc;
  function DepthFirst(goalReached, unreachable, level, frontier, path) {
    this.goalReached = goalReached;
    this.unreachable = unreachable;
    this.level = level;
    this.frontier = frontier;
  ...
  DepthFirst.builtin$cls = "DepthFirst";
  if (!"name" in DepthFirst)
    DepthFirst.name = "DepthFirst";
  $desc = $collectedClasses.DepthFirst;
  if ($desc instanceof Array)
    $desc = $desc[1];
  DepthFirst.prototype = $desc;
  function AStar(fValues, goalReached, unreachable, level, frontier, path) {
  ...
  function AStar_retrieveFrontier_closure(this_0) {
    this.this_0 = this_0;
  }
  AStar_retrieveFrontier_closure.builtin$cls = "AStar_retrieveFrontier_closure";
  if (!"name" in AStar_retrieveFrontier_closure)
    AStar_retrieveFrontier_closure.name = "AStar_retrieveFrontier_closure";
  $desc = $collectedClasses.AStar_retrieveFrontier_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  AStar_retrieveFrontier_closure.prototype = $desc;
  function AStar_retrieveFrontier__calcFValue(this_1) {
    this.this_1 = this_1;
  }
  AStar_retrieveFrontier__calcFValue.builtin$cls = "AStar_retrieveFrontier__calcFValue";
  if (!"name" in AStar_retrieveFrontier__calcFValue)
    AStar_retrieveFrontier__calcFValue.name = "AStar_retrieveFrontier__calcFValue";
  $desc = $collectedClasses.AStar_retrieveFrontier__calcFValue;
  if ($desc instanceof Array)
    $desc = $desc[1];
  AStar_retrieveFrontier__calcFValue.prototype = $desc;
  function Vector(x, y, _vector$_length) {
    this.x = x;
    this.y = y;
    this._vector$_length = _vector$_length;
  }
